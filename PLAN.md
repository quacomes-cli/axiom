...
## FAZ 5 — Derin agent modu (çok-adımlı otonom görevler)

Bugün: `chatStore.send` tek tur — model araç bloğu üretir, yürütülür, sonuç döner, biter.
Hedef: model bir HEDEF alır, plan yapar, araçları zincirler, ara sonuçları gösterir.

### 5.1 Çekirdek döngü (`src/lib/agentLoop.ts` — yeni)
- [ ] `runAgent(goal, opts)`: (1) planlama turu — model adım listesi üretir (JSON);
      (2) yürütme döngüsü — her adımda araç çağrıları, sonuç bir sonraki adımın
      bağlamına; (3) sentez turu — nihai rapor. Mevcut `send` araç yürütme yolunu
      (parseToolBlocks/executeToolBlock + native tool_calls) AYNEN kullanır.
- [ ] Sınırlar: max adım (ayar, varsayılan 8), adım başına araç zaman aşımı,
      toplam süre tavanı; `stopGeneration` döngüyü keser.
- [ ] Güvenlik: onay kapıları değişmez — interaktif oturumda confirm kartı,
      arka planda `interactive:false` (sessiz red + modele açıklama).

### 5.2 UI — plan/adım kartları
- [ ] `AgentRunCard.tsx`: sohbet içinde canlı plan (adımlar ✓/⏳/✗), genişleyebilir
      ara çıktılar, durdur butonu. ThinkingBlock/ToolBlock desenleri örnek.
- [ ] Tetikleme: chat input'ta `/agent <hedef>` + TaskBoard "agent'a devret" bu
      döngüye bağlanır (bugünkü tek-prompt devir yerine).

### 5.3 Arka plan yürütme
- [ ] `useTaskScheduler` agent görevleri `runAgent`'ı kullanır; bitince
      NotificationCenter bildirimi + sonuç TaskBoard karta işlenir.
- Kabul: "X klasöründeki dosyaları incele, özet çıkar, rapor yaz" tarzı 3+ araçlı
  hedef uçtan uca; adımlar canlı görünür; durdurulabilir; onay kapıları çalışır.
