...
export default function Sidebar(props: { onNavigateSettings?: () => void }) {
  const [passInput, setPassInput] = createSignal(localStorage.getItem("axiom_mobile_master_passphrase") || "");
  ...
  const handleDecrypt = async (e: Event) => {
    ...
    const ok = await tryDecryptCloudKeys(passInput().trim());
    if (ok) {
    ...
    <>
      ...
      >
          ...
          <div class="flex items-center gap-3 min-w-0">
            ...
            <div class="flex flex-col min-w-0">
              <span class="text-[0.92rem] font-semibold text-text truncate">
                {user()?.displayName || "Axiom Kullanıcısı"}
              </span>
        ...
        <div class="flex-1 overflow-y-auto p-4 space-y-6">
          ...
          {/* Section 1: E2EE Master Key Decryption */}
          <Show when={user() && !user()?.isAnonymous}>
            <div class="space-y-2.5">
              ...
              <form onSubmit={handleDecrypt} class="space-y-2">
                ...
                <div class="flex gap-2 items-center justify-between">
                  ...
                  >
                    {cloudKeysLoading() ? "Çözülüyor..." : decryptedKeys() ? "Güncelle" : "Deşifre Et"}
                  </button>
                </div>
            ...
            <Show
              ...
              fallback={
                <div class="rounded-xl border border-border/60 bg-surface-2/10 p-3 text-center text-text-faint text-[0.78rem] leading-relaxed">
                  Çevrimiçi bilgisayar bulunamadı.
                </div>
              }
            >
